	...
	return (
		<div id='SearchPage'>
			...
			<div className='container mx-auto' style={styles.body}>
				...
				<div className='mx-auto p-2' style={styles.searchTitle}>Cari apa, tanya mama 🙂</div>
				<div className='mx-auto' style={{ width: '85%', display: 'flex', flexDirection: 'row', gap: 10 }}>
					<input className='form-control px-2 py-1'
						style={{ height: '53px', borderRadius: 50, borderWidth: 2 }}
						type="search" name="search" id="search" placeholder='Mau cari resep apa...?'
						onChange={e => setSearch(e.target.value)} />
					<button className='btn' onClick={handleTanyaButton} style={styles.searchButton}>Tanya</button>
				</div>
			</div>
									...
									return (
										<div key={index} className="col-lg-4 col-md-5 col-sm-6 col-xs-12 p-3 ">
											<Link to={`/detail/${String(recipe.title).split(" ").join("-").toLowerCase()}`}>
												<div style={{ ...styles.resultCard, backgroundImage: `url(${recipe.image})` }}>
													<div className='p-3'
														style={styles.resultCardTitleContainer} >
														<p style={styles.resultCardTitle}>
															{recipe.title}
														</p>
													</div>
												</div>
